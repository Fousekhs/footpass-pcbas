"""W&B sweep agent entry point for the graph model variant.

Usage — two modes:

1. **Create + run** (sweep.py creates the sweep and immediately starts an agent):

    python scripts/graph/sweep.py \\
...
via ``wandb.config`` and applied through ``scripts.graph.train._build_argparser``'s
...
:mod:`pcspot.train.sweep_common`; this script only wires it to the graph
