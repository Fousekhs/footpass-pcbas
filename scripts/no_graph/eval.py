"""Standalone checkpoint evaluation for the no-graph model variant.

Reconstructs :class:`pcspot.models.no_graph_model.NoGraphSpottingModel`
from a checkpoint's ``run.json`` (written by ``scripts/no_graph/train.py``)
...
Example::
    ...
    python scripts/no_graph/eval.py \\
        --checkpoint checkpoints/no_graph/run1/best.pt \\
...
# Restricted to the keys NoGraphSpottingModel.__init__ accepts; mined
# from run.json's "args" payload (CLI overrides win). Notably smaller
# than the graph variant's set — no knn / edge / zone / radius keys.
MODEL_INIT_KEYS = (
    "hidden_dim",
    "num_classes",
    "num_mstcn_stages",
...
def main() -> int:
    ...
    metrics = evaluate_checkpoint(
        ...
        splits=args.splits,
        split=args.split,
        model_cls=NoGraphSpottingModel,
