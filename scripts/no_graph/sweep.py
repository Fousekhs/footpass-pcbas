"""W&B sweep agent entry point for the no-graph model variant.

Usage — two modes:

1. **Create + run** (sweep.py creates the sweep and immediately starts an agent):

    python scripts/no_graph/sweep.py \\
        --sweep-config configs/sweeps/no_graph.yaml \\
        --splits data/splits.json \\
        --output-dir checkpoints/sweeps/no_graph \\
...
via ``wandb.config`` and applied through ``scripts.no_graph.train._build_argparser``'s
...
:mod:`pcspot.train.sweep_common`; this script only wires it to the no-graph
